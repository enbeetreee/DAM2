...
document.getElementById("reloj").addEventListener('click', () => {
    if (horaAM) {
        horaAM = !confirm("¿Cambiar el formato a 24 horas?")

    } else {
        horaAM = confirm("¿Cambiar el formato a 12 horas (AM/PM)?")
    }
        ...
        if (editando) {
            ...
            if (existeEvento(diaEvento)) {//comprueba si el dia tiene eventos
                if (arrayEventos.length == 3) {
                    alert("No pueden añadirse más de 3 eventos por día");
                    noMas = true;
                } else {
                    eventoActual += "_" + arrayEventos.length
...
function leer() {
    if (fs.existsSync('./files/eventos.json' )) {
        let aux = JSON.parse(fs.readFileSync('eventos.json', 'utf8'));
